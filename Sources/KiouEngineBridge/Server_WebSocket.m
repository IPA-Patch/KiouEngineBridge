...
//   - A serial GCD queue funnels every KiouWsServerPush() through one
...
// process, which matches the way Tweak.m calls KiouWsServerStart() once.
// ---------------------------------------------------------------------------
...
// KiouWsServerSetTextHandler() at constructor time. Reads happen on
...
static void ws_client_recv_loop(int fd) {
    while (1) {
        ...
        if (op == 0x8) {                                 // close
            ...
            // ws_send_text (from KiouWsServerPush -> accept queue) and
